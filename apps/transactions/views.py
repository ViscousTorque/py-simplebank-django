...
class CreateTransferView(APIView):
    ...
    @extend_schema(
        ...
        "Both accounts must have the same currency and the user must own the source account.",
    )
    def post(self, request):
        ...
        logging.info("from_account=%s", from_account)
        ...
        if from_account_obj.owner != user:
            return Response(
                {"error": "Unauthorized to transfer from this account"},
                status=status.HTTP_401_UNAUTHORIZED,
            )

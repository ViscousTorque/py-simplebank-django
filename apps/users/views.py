...
class VerifyEmailView(APIView):
    @extend_schema(
        ...
        description="Validates the email and secret code."
        " Marks user as verified if correct and not used before.",
    ...
    def post(self, request):
        ...
        if not email_id or not secret_code:
            return Response(
                {"error": "Email ID and secret code are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        try:
            verify_email = VerifyEmail.objects.get(
                email=email_id, secret_code=secret_code, is_used=False
            )
